...
public class AiGeneratorService {
    ...
    public Mono<WorkflowProposals> generateWorkflowProposals(GenerateProposalsRequest request) {
        ...
        // return llm_orchestrator.generateWorkflows(request.getApi_structure(),
        // request.getHints())
        ...
        return llm_orchestrator.generateWorkflows(cleaned_request.getApi_structure(), cleaned_request.getHints())
                .doOnSuccess(proposals -> saveHistory("GENERATE_PROPOSALS", null,
                        System.currentTimeMillis() - start_time, null)
                        .subscribe())
                .onErrorResume(error -> {
                    log.error("❌ LLM Generation failed ({}). No fallback allowed.", error.getMessage());
                    // On propage l'erreur pour que l'utilisateur sache que l'IA a échoué
                ...
                .flatMap(config -> config_validator.validate(config)
                        .flatMap(report -> {
                            ...
                            return saveGeneratedConfig(config, report, request.getApi_structure())
                                    .flatMap(saved_entity -> saveHistory("GENERATE_CONFIG", saved_entity.getConfig_id(),
